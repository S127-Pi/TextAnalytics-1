...
write.csv(word_embeddings, file = 'output.csv')
```
...
# Load the dataset
data <- fread("output.csv") # Adjust the path to your actual file

# Remove the first column if it's just an index
data <- data[,-1,with=FALSE]

# Transpose the data to align with the cosine function expectations
data_t <- data

# Calculate the cosine similarity matrix for the entire set of words
similarity_matrix <- cosine(as.matrix(data_t))
...
word1 <- colnames(data_t)[max_pos[1, 1]]
word2 <- colnames(data_t)[max_pos[1, 2]]
